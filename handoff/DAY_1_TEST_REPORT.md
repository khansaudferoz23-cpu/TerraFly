...
|---|---:|---:|---|
| Backend safe upload, adapter, artifacts, and GeoTIFF suite (13 tests) | PASS | 0.87 s | final revised pytest output; artifact response hashes and TIR warning asserted |
| Frontend scientific-contract and file-selection components (2 tests) | PASS | 1.75 s | clean isolated Vitest output |
| Strict TypeScript/Vite production build | PASS | 0.284 s | final revised production build output |
...
| Browser upload/progress/result/Three.js workflow | PASS | ~8 s inference | downloaded `tir_100m_512.png`; real model, visible TIR warning, WebGL viewer, and screenshots in task outputs |
| Phone-size responsive layout | PASS | interactive | 390×844 viewport; document width did not exceed viewport width |
| Windows launcher syntax/readiness path | PASS | <1 s parser | PowerShell parser found no errors; launcher auto-opens after readiness |
...
| Extracted revised source archive smoke | PASS | pending final archive refresh | fresh path-with-spaces verification is repeated after the final commit |
