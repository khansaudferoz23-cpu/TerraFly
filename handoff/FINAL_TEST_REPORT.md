...
| Backend scientific/safety suite | PASS | 33 tests, including canonical/display separation, spike cleanup, roof flattening, direction, and raised-roof GLB regressions |
...
The final `scripts\smoke_final_workflow.py --device cuda` rerun completed in 25.335 seconds with the prepared local cache.
...
- All 33 backend tests passed in the repaired source using the prepared test environment; the updated source/Windows archives are generated under `outputs\geometry-repair-2026-08-26` after the final commit.
...
This report proves software behavior, the repaired height convention, canonical/display separation, edge-aware visual cleanup, non-stretched steep-face material handling, the separated structure layer, metric viewer sampling, and the synthetic calibration oracle. It does not prove real-world height accuracy, semantic building reconstruction, recovered façade imagery, or disaster prediction. No compatible independently surveyed remote-sensing height dataset was supplied, and strong scene-wide perspective tilt remains diagnosed rather than corrected.
