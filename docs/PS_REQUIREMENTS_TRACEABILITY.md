...
|---|---|---|---|
| PNG/JPG input | Safe RGB intake followed by real Depth Anything V2 Large inference and a normalized relative surface | API, adapter-contract, artifact, and real CUDA smoke tests | Output is an rDSM/relative surface proxy, never metres |
...
| Pretrained monocular backbone | `depth-anything/Depth-Anything-V2-Large-hf`, with exact revision recorded, CUDA preference, and CPU fallback | Real CPU/CUDA checks and manifest provenance | It is a general relative-depth baseline with a non-commercial checkpoint licence, not a satellite-height oracle |
...
| Textured 3D terrain | Original optical texture is projected onto an orientation-tested Three.js heightfield; a separate RGB-guided display grid cleans spikes/noise and flattens conservative raised regions; steep triangles use neutral wall material instead of stretched top-down pixels; GLB embeds the source PNG with UVs, unit normals, and lit PBR materials | Geometry/UV/normal/material-group tests, display-separation regression, production build, and real scene run | The cleaned display grid is downsampled and is not the scientific raster; neutral walls are synthetic, not recovered façades |
...
| Accuracy metrics | Calibration reports held-out RMSE/MAE/bias/R² and coverage; a separate model benchmark emits RMSE, MAE, bias, and Pearson correlation only from matching held-out GeoTIFF grids | Calibration/evaluation tests and metric-card generator | The real model card remains blocked until licensed independent RGB–DSM/nDSM truth is acquired; synthetic near-zero error proves plumbing only |
