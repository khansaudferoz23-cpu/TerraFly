...
| Textured 3D terrain | Original optical texture is projected onto an orientation-tested Three.js heightfield; a separate RGB-guided display grid cleans spikes/noise and flattens conservative raised regions; steep triangles use neutral wall material instead of stretched top-down pixels; GLB export matches this policy | Geometry/UV/material-group tests, display-separation regression, production build, and real scene run | The cleaned display grid is downsampled and is not the scientific raster; neutral walls are synthetic, not recovered façades |
